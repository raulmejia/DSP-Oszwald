...
path_to_your_annotation_file <- "/media/rmejia/mountme88/Projects/DSP/Data/Annotation/20200625_CTA_annot/20200625_DSP_CTA_annot_ordered_no_16_1.tsv"
# In your annotation file should include the following column names
# Unique_ID, Scan_ID, ROI_ID, Biopsy_year, Histology_number, ID, Morphological_Categories 
...
column_to_cut_hereafter_in_your_table <- "A2M"
...
###################################
table <- read.table( path_to_your_table_file , sep = "\t", header = TRUE , stringsAsFactors = FALSE)
annot <- read.table( path_to_your_annotation_file , sep = "\t", header = TRUE , stringsAsFactors = FALSE)

Unique_IDs_table_vs_annot <- all(table[,"Unique_ID"] == annot[,"Unique_ID"])
if( Unique_IDs_table_vs_annot == TRUE ){
    print("OK... Unique_IDs from table and annot match in exactly the same order")
}else{
  print("ERROR!... Unique_IDs from table and annot don't match")
}

###################################
#### Extracting the Raw expression matrix
###################################
colpositions_withgenes <- grep( column_to_cut_hereafter_in_your_table , colnames( table )):dim(table)[2]
...
# Testing for negatives probes and prunning

if( length(grep("NegPr",rownames(Raw_expmat_Noised)))>=1  ){
  cutoff_val <- mean(Raw_expmat_Noised[grep("NegPr",rownames(Raw_expmat_Noised)),]) + 2*sd(Raw_expmat_Noised[grep("NegPr",rownames(Raw_expmat_Noised)),])
  row_indices <- apply( Raw_expmat, 1, function( x ) any( x > cutoff_val ) )
  Raw_expmat <- Raw_expmat_Noised[row_indices,]
  if( length(grep("NegPr",rownames( Raw_expmat )))>=1 ){
    Raw_expmat <- Raw_expmat[- grep("NegPr",rownames( Raw_expmat )) , ]  
  } 
  print(paste0("You have_",length(grep("NegPr",rownames(Raw_expmat_Noised))), "_negative probes,and they were used to set the treshold > mean + 2*sd "))
  print(paste0("Your original data frame had_", dim(Raw_expmat_Noised)[1] ,"_rows after prunning it has_", dim(Raw_expmat)[1] ))
}else{
  Raw_expmat <- Raw_expmat
}
...
paste0( path_Results_directory,"/Preprocessing","/",paste0( data_label, "Log2_CycLoessBatchSep_combat" ),".tsv" )
...
head(combat_cyclic_loess_BatchSep )
dev.off()
